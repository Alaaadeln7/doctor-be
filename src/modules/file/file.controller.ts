...
export class FileController {
  ...
  async uploadMyDoctorProfileAuthFiles(@Req() req: Request, @UploadedFiles() files: FilesType) {
    const { id } = req['user'];
  ...
  async uploadMyDoctorProfileClinicFiles(
    ...
    if (req.body.keepFiles) {
      try {
        keepFiles =
          typeof req.body.keepFiles === 'string'
  ...
  async updateMyDoctorProfileAuthFiles(@UploadedFiles() files: FilesType, @Req() req: Request) {
    const doctorId = req['user']?.id;
