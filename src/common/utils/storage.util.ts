...
export class StorageUtilService {
    ...
    return {
      streamUploader: (buffer: Buffer): Promise<CloudinaryUploadResult> => {
        return new Promise((resolve, reject) => {
          const uploadStream =
            this.cloudinaryStorageClient.uploader.upload_stream(
  ...
  private generatePublicId(
    ...
    const randomWord = [...Array(4)]
      .map(() =>
        "abcdefghijklmnopqrstuvwxyz1234567890".charAt(
  ...
  async uploadFile(fileObj: FileObjType, bucket: string) {
    ...
    const generatedPublicId = this.generatePublicId(
      bucket,
      originalname,
      fieldname
    );
    const uploader = this.makeCloudinaryStream(
      bucket,
      generatedPublicId,
      mimetype
    );
  ...
  async uploadFiles(filesObjs: FileObjType[], bucket: string) {
    const uploads = filesObjs.map((fileObj) =>
      this.uploadFile(fileObj, bucket)
    );
    ...
    if (uploadedFiles.length < filesObjs.length) {
      ...
      const successUploadedFilesPublicIdsList = uploadedFiles.filter(
        (upload) => upload && upload.public_id
      );
      if (successUploadedFilesPublicIdsList.length > 0) {
        await this.destroyFiles(
          successUploadedFilesPublicIdsList.map((file) => file.public_id),
          bucket
        );
      }
  ...
  async destroyFiles(publicIdList: string[], bucket: string) {
    if (!publicIdList || publicIdList.length === 0) {
      throw new Error(
        "Public ID list is empty. Bad usage of destroyFiles method!"
      );
    }
